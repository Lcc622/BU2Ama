...
from app.config import DEFAULT_TEMPLATE_TYPE, RESULTS_DIR, STORE_CONFIGS, TEMPLATES, UPLOADS_DIR
...
def _is_allowed_data_filename(filename: str) -> bool:
    ...
    allowed_excel = (
        upper_name.startswith("DA-") or upper_name.startswith("PZ-")
    ) and upper_name.endswith((".XLSM", ".XLSX"))
    allowed_report = upper_name in {"DM-ALL+LISTINGS+REPORT.TXT", "PZ-ALL+LISTINGS+REPORT.TXT"}
    return allowed_excel or allowed_report
...
async def analyze_file(file: UploadFile = File(...)):
    ...
    try:
        if not _is_allowed_data_filename(file.filename):
            raise HTTPException(
                status_code=400,
                detail="仅支持上传 DA-/PZ- 数据文件和 DM/PZ Listings Report"
            )
        # 保存上传的文件
...
async def get_store_info():
    ...
    return {
        "store_group": "DM_PZ",
        "display_name": "DM/PZ 店铺",
    }


@router.get("/files", response_model=List[FileInfo])
async def list_files():
    """列出已上传的文件（仅保留 DM/PZ 实例允许的文件）"""
    files = []
    for file_path in UPLOADS_DIR.glob("*"):
        if not file_path.is_file():
            continue
        if not _is_allowed_data_filename(file_path.name):
            continue
        stat = file_path.stat()
