...
STORE_PREFIX_PATTERN = re.compile(r"^(DM|DA|PZ)-\d", re.IGNORECASE)
...
class FollowSellProcessor:
    ...
    def __init__(self):
        ...
        if legacy_db_path.exists():
            print("警告: 检测到旧索引库 ep_index.db，请重新上传店铺数据源以重建 DM/PZ 分店铺索引。")
        self._load_mapping()
        # 不在初始化时加载店铺索引，按店铺懒加载
    ...
    def _ensure_index_tables(self, conn: sqlite3.Connection) -> None:
        ...
        conn.execute(
            ...
            CREATE TABLE IF NOT EXISTS ep_sku_index (
                store_prefix TEXT NOT NULL DEFAULT 'DM',
                style TEXT NOT NULL,
                sku TEXT NOT NULL,
                size TEXT NOT NULL,
        ...
        if "store_prefix" not in columns:
            conn.execute("ALTER TABLE ep_sku_index ADD COLUMN store_prefix TEXT NOT NULL DEFAULT 'DM'")
    ...
    def _normalize_suffix(self, suffix: str) -> str:
        ...
        if normalized == "-":
            return "-PL"
        if normalized in {"-PL", "-PLA", "-PLB", "-PLC"}:
            return "-PL"
        if normalized in {"-PLPH", "-PLPHA", "-PLPHB", "-PLPHC"}:
            return "-PLPH"
        if normalized in {"-DA", "-DAA", "-DAB", "-DAC"}:
            return "-DA"
        if normalized in {"-DAPH", "-DAPHA", "-DAPHB", "-DAPHC"}:
            return "-DAPH"
        return normalized
    ...
    def find_sizes_for_skc(self, skc: str, template_type: str = "DaMaUS") -> Dict:
        """根据 SKC 查找所有尺码信息
    ...
    def process_skc(self, skc: str, store_prefix: str = "DM") -> Dict[str, Any]:
