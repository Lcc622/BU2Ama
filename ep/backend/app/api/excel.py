...
def _is_allowed_data_filename(filename: str) -> bool:
    ...
    if not normalized:
        return False
    if normalized in STORE_CONFIGS["EPUS"]["source_files"]:
        return True
    return normalized == STORE_CONFIGS["EPUS"]["listing_report"]
...
async def analyze_file(file: UploadFile = File(...)):
    ...
    try:
        if not _is_allowed_data_filename(file.filename):
            raise HTTPException(
                status_code=400,
                detail="仅支持上传 EP-0/EP-1/EP-2.xlsm 和 EP-All+Listings+Report.txt 数据文件"
            )
        # 保存上传的文件
...
async def list_files():
    ...
    for file_path in UPLOADS_DIR.glob("*"):
        if not file_path.is_file():
            continue
        if not file_path.name.upper().startswith("EP-"):
            continue
        stat = file_path.stat()
